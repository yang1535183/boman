...
<div id="updateDialog" class="crudDialog">
	<form id="updateForm" method="post">
		...
		<div class="form-group">
			<label for="pid" class="active">上级组织</label>
			<select id="pid" name="pid" class="form-control">
					<option value="0">请选择上级</option>
					<c:forEach var="upmsSystem" items="${upmsSystems}">
					<option value="${upmsSystem.organizationId}" <c:if test="${organization.pid==upmsSystem.organizationId}">selected="selected"</c:if>>${upmsSystem.name}</option>
					</c:forEach>
			</select>
		</div>
