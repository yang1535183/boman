...
var ns = function () {
    ...
    return {
        ...
        alert: function (content, ok, options) { //提示
            if (!top.ns.view.Dialog)
                top.ns.requireJS("/resources/form/view/dialog.js");
            var id = "__divAlert" + new Date().getTime();
            var def = {
                id: id,
        ...
        success: function (title, msg, fun) {
            if (typeof(top.swal) !== "function") {
                top.ns.requireCSS("/resources/form/sweetalert/sweetalert.css");
                top.ns.requireJS("/resources/form/sweetalert/sweetalert.min.js");
            }
            if ($.isFunction(msg)) {
                fun = msg;
                msg = "";
