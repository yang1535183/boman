...
var KisBpmChoseAssignmentCtrl = ['$scope', '$http', function($scope, $http) {
        ...
        $http({
            ...
            .success(function (data, status, headers, config) {
                ...
                for (var i=0; i<obj.length; i++) {
                    ...
                    roles.push({id:obj[i].roleId,name:obj[i].title});
                }
                $scope.roles = roles;
            })
    ...
    function getDetail(value,page,limit)
    {
        ...
        $http({
            ...
            headers: {'Accept': 'application/json',
                'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'},
            url: '/manage/user/getActUser?roleId='+value+"&page="+page+"&limit="+limit})
            .success(function (data, status, headers, config) {
                //封装数据
                var obj = data.users;
                if (data != null) {
                    var accounts = [];
                    for (var i=0; i<obj.length; i++) {
                        accounts.push({id:obj[i].userId, code : obj[i].username, name : obj[i].realname, index:i});
                    }
