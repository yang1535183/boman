package com.boman.upms.server.controller;
...
import com.boman.common.util.wechat.wechat.factory.AccessTokenFactory;
...
import com.boman.upms.client.shiro.session.UpmsSession;
import com.boman.upms.client.shiro.session.UpmsSessionDao;
...
import com.boman.upms.rpc.api.UpmsSystemService;
...
public class SSOController {
    ...
    public Object login(HttpServletRequest request, HttpServletResponse response, ModelMap modelMap) {
        ...
        if (StringUtils.isBlank(hasCode)) {
            ...
            // 保存username-sessionId
            RedisUtil.hset(ZHENG_UPMS_SERVER_SESSION_USER, username, sessionId);
            ...
            if(userAgent.indexOf("micromessenger")>-1){
                RedisUtil.hset("apps", sessionId.toString(), sessionId.toString());
                RedisUtil.set(ZHENG_UPMS_SERVER_SESSION_ID + "_" + sessionId, code, 259200000);
                // code校验值
                RedisUtil.set(ZHENG_UPMS_SERVER_CODE + "_" + code, code, 2592000);
            }else {
        ...
        if(userAgent.indexOf("micromessenger")>-1){//微信客户端
            ...
            //logger.info("==============[OAuthServlet]获取网页授权code=" + code + "==============[OAuthServlet]获取网页跳转权限=" + scope);
            if (null != code && !"".equals(code)) {
                ...
                if (!"".equals(oa) && null != oa) {
                    ...
                    boolean flag=upmsUserService.haveOpenId(oa.getOpenId());
                    if(!flag){
                        if(!"".equals(oa.getOpenId()) && oa.getOpenId()!=null){
                            //未绑定 去绑定
                            upmsUserService.insertOpenId(oa.getOpenId(),username);
                        }
                    }
                    backurl="http://zsfw.qss.gov.cn/wechat/zhaoshang/oauth";
                }
            }
