package com.boman.upms.client.shiro.filter;
...
import com.boman.upms.client.util.RequestParameterUtil;
import com.boman.upms.client.util.SerializableUtil;
import com.boman.upms.common.constant.UpmsConstant;
    ...
    private boolean validateClient(ServletRequest request, ServletResponse response) {
        ...
        if (StringUtils.isNotBlank(cacheClientSession)) {
            ...
            if(userAgent.indexOf("micromessenger")>-1){
                RedisUtil.set(ZHENG_UPMS_CLIENT_SESSION_ID + "_" + sessionId, cacheClientSession, 2592000);
                LOGGER.info("micromessenger code校验正确, 有效期：{}", 2592000);
            } else {
                LOGGER.info("没有进入 micromessenger code校验正确, 有效期：{}", timeOut);
            ...
            try {
                ...
                if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                    ...
                    if (1 == result.getIntValue("code") && result.getString("data").equals(code)) {
                        // code校验正确，创建局部会话
                        if(userAgent.indexOf("micromessenger")>-1){
                            RedisUtil.set(ZHENG_UPMS_CLIENT_SESSION_ID + "_" + sessionId, code, 2592000);
                            LOGGER.info("micromessenger code校验正确, 有效期：{}", 2592000);
                        }else {
