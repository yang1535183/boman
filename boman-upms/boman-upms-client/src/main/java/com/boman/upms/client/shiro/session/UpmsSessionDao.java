package com.boman.upms.client.shiro.session;
...
public class UpmsSessionDao extends CachingSessionDAO {
    ...
    protected void doUpdate(Session session) {
        ...
        if(RedisUtil.hget("apps", session.getId().toString()) != null){
            session.setTimeout(-10000);
            RedisUtil.set(ZHENG_UPMS_SHIRO_SESSION_ID + "_" + session.getId(), SerializableUtil.serialize(session), 2592000);
        }else {
    ...
    protected void doDelete(Session session) {
        ...
        // 删除session
        RedisUtil.hremove(ZHENG_UPMS_SERVER_SESSION_USER, username);
        RedisUtil.remove(ZHENG_UPMS_SHIRO_SESSION_ID + "_" + sessionId);
        LOGGER.info("doUpdate >>>>> sessionId={},{}", sessionId,session.getTimeout());
    }
    ...
    protected Serializable doCreate(Session session) {
        ...
        LOGGER.info("doCreate >>>>> sessionId={},Timeout{}", sessionId,session.getTimeout());
        return sessionId;
    }
