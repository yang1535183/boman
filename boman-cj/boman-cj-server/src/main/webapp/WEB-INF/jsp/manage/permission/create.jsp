	...
	if (systemId != 0) {
		$.getJSON('${basePath}/manage/permission/list', {systemId: systemId, type: pidType, limit: 10000}, function(json) {
			...
			if(type == 1){
                ...
                $('.type4').show();
                pidType = 1;
                $.getJSON('${basePath}/manage/permission/threeMenu',{systemId: systemId, type: pidType, pId: pid, limit: 10000}, function (json) {
                    var datas = [{id: 0, text: '请选择上级'}];
                    for (var i = 0; i < json.rows.length; i ++) {
                        var data = {};
                        data.id = json.rows[i].permissionId;
...
function initMid(id) {
	if(id != 0){
        $.getJSON('${basePath}/manage/permission/midlist',{systemId: systemId, type: pidType, limit: 10000, pid: id}, function (json) {
            var datas = [{id: 0, text: '请选择上级'}];
            for (var i = 0; i < json.rows.length; i ++){
                var data = {};
                data.id = json.rows[i].permissionId;
